...
template <class T> reframework::API::ManagedObject* to_managed_object(REArray<T>* array) {
...
template <class... Args> std::string str_call(reframework::API::ManagedObject* obj, std::string_view name, Args... args) {
    const REString* res = obj->call<REString*>(name, reframework::API::get()->get_vm_context(), obj, args...);
    return res->to_str();
}

template <class... Args> std::string str_call(reframework::API::Method* method, Args... args) {
    const REString* res = method->call<REString*>(reframework::API::get()->get_vm_context(), args...);
    return res->to_str();
}
...
template <class T = uint32_t> std::string get_enum_name(std::string_view name, T value) {
    ...
    *inst->get_field<T>("value__") = value;
    return call<REString*>(inst, "ToString()")->to_str();
}
...
template <class T = uint32_t> std::string get_enum_name(reframework::API::ManagedObject* value) {
    return call<REString*>(value, "ToString()")->to_str();
}
...
REString* create_managed_string(std::string_view string);
